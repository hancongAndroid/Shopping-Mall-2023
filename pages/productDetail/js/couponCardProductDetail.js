...
function getProductInfo() {
	http('GET', `/mallv2/product/getProductInfoById`, {
		...
		$('.originalPrice, #originalPricePopup').text(productInfo.curPrice)
		$('#volumeStr').text(volumeStr(productInfo.volumeStr))
		// $('.activityMax').text(activityMax/10)
		$('.activityMax').text(productInfo.curPrice-productInfo.price)
...
function getRecommondList() {
	http('GET', `/mallv2/h5/activity/queryListByPage`, {
		...
		$.each(productList, (i, n) => {
			if (i < 6) {
				recommond1 += `<div class="product-item" onclick="window.open('./guessProductDetail.html?productId=${n.id}', '_self')">
				...
				recommond2 += `<div class="product-item" onclick="window.open('./guessProductDetail.html?productId=${n.id}', '_self')">
								<img class="product-image" src="${n.productImg}" />
								<div class="product-info">${n.productName}</div>
								<div class="product-btm">
									<!--<span class="product-jifen">${parseInt((n.originalPrice - n.price) * 10)}积分</span>-->
		...
		$.each(fakeUsers, (i, n) => {
			user += `<div class="swiper-slide">
							<div class="random">
								...
								<div class="random-name">${n.nickname} 在${Math.ceil(Math.random() * 10)}分钟前使用专享券抢购了该商品</div>
							</div>
						</div>`
		})
...
function appendHtml() {
        ...
        $('#couponText').text(couponText.text)
    let html = `
...
<div class="coupon-dic2">
    ...
    <div class="price-box">
        ...
        <div class="right item">
            ...
            <div class="text-box">
                <span>商品券</span>
                <span>${couponText.text1}</span>
                <span>${couponText.text2}</span>
            </div>
        </div>
    </div>
</div>
<div class="coupon-bg">
    ...
    <div class="left">
        <div class="coupon-bg-price">
            <i class="icon">￥</i><span>${couponText.num}</span>
        </div>
        <div class="coupon-bg-text">
            <p class="text1">${couponText.text1}使用</p>
