...
function getProductInfo() {
	http('GET', `/mallv2/product/getProductInfoById`, {
		...
		$('#originalPricePopup, #curPrice').text(productInfo.curPrice)
		...
		$('#priceYuan, #priceYuanPopup').text(productInfo.price)
...
$(function () {
	...
	// 购买按钮
	$('#shopBtn1').on('click', function() {
		$('#buyPopup').addClass('show')
	})
	// 购买按钮
	$('#shopBtn').on('click', function() {
		if (wallerAmount < activityMax) {
			layer.open({
				title: '温馨提示'
				,content: `您的余额不足，请尽快获取`
				,btn: ['确定','获取余额']
				,yes: function(index){
					layer.close(index)
					clikStatistics('QBGM_QD')
				},no: function(index){
					layer.close(index)
					window.location.href = '../coupon/index.html'
					clikStatistics('QBGM_QEF')
				}
			})
			return
		}
