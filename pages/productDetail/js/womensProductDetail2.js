...
let productId = getUrlKey('productId') || 9785 //11699
let specMap = {}	//查规格信息
...
// 非女装商品样式
let columnId = getUrlKey('columnId')
if (columnIdList.includes(+columnId)) {
	// $('.oldForNew-product-info-2').addClass('dn')
	$('.label-bar').addClass('dn')
	$('.shop-right-text2').addClass('dn')
	$('.oldForNew-product-info .curPrice').addClass('priceReal').removeClass('curPrice')
	$('#shopBtn .btn-l .curPrice').addClass('originalPrice').removeClass('curPrice')
	$('#shopBtn .btn-l .txt').text('日常价')
	$('#shopBtn .btn-r .txtcon').html(`女王专享价<br>&nbsp;&nbsp;&nbsp;￥<span class="priceReal">0</span>`)
	$('#shopBtn .btn-r .counter').addClass('dn')
	$('#shopBtnCoupon').addClass('dn')
	$('.now-price-wrapper .text1').text('女王专享价:')
	$('.tips-bar .txt').text('专享特惠 · 时尚精选')
	$('#discountWomens').addClass('dn')
}
...
function getProductInfo() {
	http('GET', `/mallv2/product/getProductInfoById`, {
		...
		subjectId = productInfo.subjectId
		if (!productInfo) {
			layer.open({
			    content: '商品已下架！'
			    ,skin: 'msg'
		...
		// 信息栏目
		$('.curPrice, #priceYuanPopup').text(productInfo.curPrice)
		...
		// 库存
		$('#stock').text(filterStock(productInfo.stock))
		...
		$('.card-bar').removeClass('dn')
		$('.curPrice').text(productInfo.curPrice)
		
		getRecommendOther()		//推荐
...
function getRecommendOther() {
	http('GET', `/mallv2/subjectsController/getProducts`, {
		...
		let recommond = res.data.list.sort(randomSort)
		...
		$.each(recommond, (i, n) => {
			if (i < 3) {
				recommond1 += `<div class="product-item" onclick="window.open('./womensProductDetail.html?productId=${n.id}&columnId=229', '_self')">
				...
				recommond2 += `<div class="product-item" onclick="window.open('./womensProductDetail.html?productId=${n.id}&columnId=229', '_self')">
								<img class="product-image" src="${n.productImg}" />
								<div class="g-product-info">
									<div class="descr">${n.productName}</div>
									<div class="integral-price">
...
function getProductList() {
	...
	http('GET', `/mallv2/subjectsController/getProducts`, {
		...
		// 数据渲染
		productListWomens(productList, $('#productList'),null,'detail')
	}, err => {
		console.log(err)
	})
}
...
$(function () {
	...
	// 购买按钮
	$('#shopBtn, #shopBtn2, #shopBtn3, #shopBtn4, #subSubmit, #shopBtnCoupon').on('click', function() {
