...
let productId = getUrlKey('productId') || 13940//10776//12386//9921  //8861 一种规格带无库存    //10972 两种规格都有库存    //8744 7432 两种规格带无库存
...
function getProductInfo() {
	http('GET', `/mallv2/product/getProductInfoById`, {
		...
		$('#originalPrice, #originalPricePopup, .originalPrice').text(productInfo.originalPrice)
		$('.volumeStr').text(volumeStr(productInfo.volumeStr))
		$('#activityMax, #reduceYuan, #activityMaxPopup').text(productInfo.activityMax)
		$('#priceYuan, .priceReal').text(productInfo.price)
		$('.priceRealjf').text(parseInt(productInfo.price * 10))
		...
		// let recommond1 = ''
		// let recommond2 = ''
		// $.each(productInfo.recommendOther, (i, n) => {
		// 	if (i < 6) {
		...
		// 							<span class="product-jifen">${n.activityMax}${sourcePlatform == 'mall' ? '积分' : '积分'}</span>
		// 							<span class="product-jifen-price">+${n.price}元</span>
		// 						</div>
		// 					</div>`
		// 	} else {
...
$(function () {
	...
	// 购买按钮
	$('#shopBtn, #shopBtn3').on('click', function() {
		$('#buyPopup').addClass('show')
		$('body').addClass('noscroll')
        if (!isWeapp) return
