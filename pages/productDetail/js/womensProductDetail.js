...
function getProductInfo() {
	http('GET', `/mallv2/product/getProductInfoById`, {
		...
		$('.now-price-wrapper #gzprice').text(productInfo.curPrice)
		if (!productInfo) {
			layer.open({
			    content: '商品已下架！'
			    ,skin: 'msg'
			...
			$('.specification:eq(1) .spec').each(function() {
				...
				if (specMap[specVal].stock == 0) {
					...
					if ($('.specification:eq(1) .spec.active').length == 0) {
						...
						$('#addressPirce').text(specMap[specVal].price)
						$('.now-price-wrapper #gzprice').text(specMap[specVal].curPrice)
...
function selectSpec2() {
	...
	if ($(this).hasClass('active')) {	//未选中操作
		...
		$('.now-price-wrapper #gzprice').text(specMap[specVal].gzprice)
		$('.now-price-wrapper #gzprice1').text('￥'+specMap[specVal].curPrice)
        $('#addressPirce').text(specMap[specVal].price)
...
$(function () {
	...
	// 购买按钮
	$('#shopBtn, #shopBtn2, #shopBtn3, #shopBtn4, #subSubmit, #shopBtnCoupon, #share').on('click', function() {
