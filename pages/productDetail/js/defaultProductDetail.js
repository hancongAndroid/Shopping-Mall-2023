...
let productId = getUrlKey('productId') || 5429 
// 5961 普通
// 8332 送话费
...
function getProductInfo() {
	http('GET', `/mallv2/product/getProductInfoById`, {
		...
		$('#activityMax').text(productInfo.activityMax)
		// $('#priceYuanPopup').text(productInfo.price)
		...
		// 活动类型样式
		if (activityType == 'RECHARGE_COUPON') {		// 送话费
			$('#infoColumn').css('background-color', 'rgb(89, 147, 255)')
			$('#infoColumn .hot-txt').html(`${filterVolume(productInfo.volumeStr)}人已领话费`)
			$('#infoColumn .product-yuan').html(`抄底价￥<span class="num">${productInfo.price}</span>`)
			$('#infoColumn .reduce-txt-wrap').html(`<span class="reduce-tip">严选已省</span><span class="reduce-num">${productInfo.preferentialPrice}元</spa>`)
			$('#infoColumn .reduce-txt-wrap').css('background-image', 'url(https://img.quanminyanxuan.com/service/16fc2b19b6324c9cad7b688ead8ab93f.png)')
			$('#shopBtn').html(`<span class="shop-right-txt-recharge">购买省￥<i class="preferential-price">${productInfo.preferentialPrice}</i>再赠<i class="activity-max">${productInfo.activityMax}</i>元话费</span>
								<span class="shop-right-btn-recharge">立即购买</span>`)
			$('.now-price-wrapper').html(`<span class="text3"><span class="sub-text"></span><i id="priceYuanPopup">${productInfo.price}</i>元</span>
										<span class="text5">（再送${productInfo.activityMax}元话费）</span>`)
			$('.reduce-img').removeClass('dn')
		} else {
			$('#infoColumn').css('background-color', 'rgb(207, 25, 42)')
			$('#infoColumn .hot-txt').html(`${filterVolume(productInfo.volumeStr)}人已抢购`)
			$('#infoColumn .product-yuan').html(`严选价￥<span class="num">${productInfo.price}</span>`)
			$('#infoColumn .reduce-txt-wrap').html(`<span class="reduce-tip">优惠已省</span><span class="reduce-num">${(parseInt(productInfo.originalPrice *100) - parseInt(productInfo.price * 100)) / 100}元</spa>`)
			$('#infoColumn .reduce-txt-wrap').css('background-image', 'url(https://img.quanminyanxuan.com/service/f196504ef0a447cea78551d3d746710d.png)')
			$('#shopBtn').html(`<img class="shop-right-btn" src="https://img.quanminyanxuan.com/other/9c5625afd1574302b73e6fd093f4bbef.gif" />
								<span class="shop-right-text">立即抢购</span>`)
			$('.now-price-wrapper').html(`<span class="text3"><span class="sub-text">严选价：</span><i id="priceYuanPopup">${productInfo.price}</i>元</span>
										<span class="text4 dn">￥${productInfo.originalPrice}</span>`)
		}
