...
let productId = getUrlKey('productId') || 38710
let columnId = getUrlKey('columnId') || '0'	// 商品栏目id
...
window.addEventListener('popstate', function (event) {
    if (backFlag) {
            ...
            // }else{
                window.location.href = '../memberDiscount/index.html'
            // }
    }
})
...
function getProductInfo () {
    http('GET', `/mallv2/product/getProductInfoById`, {
        ...
        $('#discount').text(((productInfo.curPrice / productInfo.originalPrice) * 10).toFixed(1) + '折')
		$('.discountAfter').text(Math.round((productInfo.curPrice - productInfo.price) * 100) / 100)
        // 评价
...
function getRecommondList () {
    http('GET', `/mallv2/h5/activity/queryListByPage`, {
        ...
        $.each(productList, (i, n) => {
            if (i < 6) {
                recommond1 += `<div class="product-item" onclick="window.open('../memberDiscountDetail/brandSaleProductDetail.html?productId=${n.id}', '_self')">
                ...
                recommond2 += `<div class="product-item" onclick="window.open('../memberDiscountDetail/brandSaleProductDetail.html?productId=${n.id}', '_self')">
								<img class="product-image" src="${n.productImg}" />
								<div class="product-info">${n.productName}</div>
								<div class="product-btm">
									<!--<span class="product-jifen">${parseInt((n.originalPrice - n.price) * 10)}积分</span>-->
        ...
        $('.specification:eq(0) .spec').each(function () {
            ...
            if (specMap[specVal].stock == 0) {
                ...
                if ($('.specification:eq(0) .spec.active').length == 0) {
                    ...
                    // 默认选中规格
                    productSpecId = specMap[specVal].id
                    $('.priceYuanPopup').text(Math.round((specMap[specVal].price) * 100) / 100)
                    $('#originalPricePopup').text(specMap[specVal].originalPrice)
                    $('#infoImgPopup, #infoImgAddress').attr('src', specMap[specVal].proImg)
                    $('#pleaseChooseSpec, #pleaseChooseSpecAddress').text(specMap[specVal].skuSpec)
                }
...
function toWeapp () {
    envType().then((res) => {
        switch (res) {
            case 1:
                window.open('../memberDiscount/index.html', '_self')
                break;
            case 2:
...
$(function () {
    ...
    $('#infoTitle,.oldForNew-product-info-4 .right').on('click', function () {
        window.open(`../productDetail/memberDiscountDetail.html?productId=${productInfo.id}`, '_self')
    })
    ...
    */
	...
	$(document).on('click', '.viewDetail', function() {
		window.location.href = '../productDetail/memberDiscountDetail.html?productId=' + productId
	})
