...
let myVideo = ''
let myBtn = ''
let userClick = false // 记录用户是否点击播放视频
let couponList = [
    {
        num: 49,
        text: '无门槛券',
...
window.addEventListener('popstate', function (event) {
    if (backFlag) {
        window.location.href = '../integralCenter/couponCard.html'
    }
})
...
function getProductInfo () {
    http('GET', `/mallv2/product/getProductInfoById`, {
        ...
        $('#priceYuanPopup,.price .priceReal').text(productInfo.price)
        $('#priceYuan').text(productInfo.curPrice)
        $('.curPrice').text(productInfo.curPrice)
        $('#discount').text(((productInfo.price / productInfo.originalPrice) * 10).toFixed(1) + '折')
        // 评价
        $('#goodRatio').text(productInfo.goodRatio)
        $('#commentSum').text(productInfo.commentSum)
        let commentNum = +productInfo.commentSum > 10000 ? '1万+' : productInfo.commentSum
        $('.commentSum').text(commentNum)
        $('.today').text(`${moment().format('M.DD')} 0点开抢`)
        ...
        let bannerImg = ''
        let loop = true
        if (productInfo.productVideo) {
            loop = false
            bannerImg = `
                            <div class="swiper-slide wrapper-video" style="display: flex;justify-content: center;align-items: center;background-color: #000;position: relative;">
                                <video class="myVideo" style="width:auto;height:7.5rem;" poster="${productInfo.productVideoCover}" src="${productInfo.productVideo}" width="100%" type="video/mp4" objectfit="contain" preload="metadata" playsinline="" webkit-playsinline="" x5-playsinline="" x5-video-player-type="h5" x5-video-player-fullscreen="true" x5-video-orientation="portraint" x-webkit-airplay="true">
                                </video>
        ...
        new Swiper('.swiper-container-banner', {
            ...
            on: {
                slideChange: function () {
                    if (myVideo && this.activeIndex == 0 && userClick) {
                        myVideo.play()
                        myBtn.addClass('dn')
                        isPlaying = true
                    } else if (myVideo && this.activeIndex != 0) {
                        myVideo.pause()
...
function appendHtml () {
    couponPrice = productInfo.curPrice - productInfo.price
    couponText = couponList.find((v) => couponPrice == v.num)
    if (!couponText) {
        $('.get-coupon').addClass('dn')
    }
    $('#couponText').text(couponText && couponText.text)
    $('#couponNum').text(couponPrice)
    let html = `
    <div class="coupon-dic">
    <div class="left item">
        <div class="price">￥${productInfo.curPrice}</div>
        <div>日常价</div>
    </div>
...
<div class="coupon-dic2">
    <div class="text">领券购买可使用以下优惠</div>
    <div class="price-box">
        <div class="left item">
            <div class="left-price">${productInfo.curPrice}</div>
            <div class="text-box"><span>活动价</span></div>
        </div>
        <div style="padding: 0 0.1rem;">-</div>
        <div class="right item">
            <div class="right-price">${couponPrice}</div>
            <div class="text-box">
                <span>商品券</span>
                <span>${couponText && couponText.text1 || ''}</span>
                <span>${couponText && couponText.text2 || ''}</span>
            </div>
        </div>
    </div>
</div>
<div class="coupon-bg">
    <div class="coupon-icon">商品券</div>
    <div class="left">
        <div class="coupon-bg-price">
            <i class="icon">￥</i><span>${couponText && couponText.num}</span>
        </div>
        <div class="coupon-bg-text">
            <p class="text1">${couponText && couponText.text1 || ''}使用</p>
            <p class="time">有效期：${moment().format('YYYY-MM-DD')}（仅限今日）</p>
        </div>
    </div>
    <div class="right" id="getBtn">
        立即领取
    </div>
</div>
    `
    $('.coupon-content').append(html)
}
// 获取 滚动用户列表 数据
// getFakeUsers()
...
function getRecommondList () {
    http('GET', `/mallv2/h5/activity/queryListByPage`, {
        ...
        $.each(productList, (i, n) => {
            if (i < 6) {
                recommond1 += `<div class="product-item" onclick="window.open('../productDetail/guessProductDetail2.html?productId=${n.id}', '_self')">
								...
								<div class="product-btm">
									...
									<span class="product-jifen-price">
                                    <span class="icon">￥</span> 
                                    <span class="num">${n.price}</span> </span>
								</div>
							</div>`
            } else {
                recommond2 += `<div class="product-item" onclick="window.open('../productDetail/guessProductDetail2.html?productId=${n.id}', '_self')">
								<img class="product-image" src="${n.productImg}" />
								<div class="product-info">${n.productName}</div>
								<div class="product-btm">
									<!--<span class="product-jifen">${parseInt((n.originalPrice - n.price) * 10)}积分</span>-->
...
function confirmOrder () {
    ...
    http('POST', `/mallv2/order/h5CreateOrder`, {
        ...
        if (res.code == 200) {
            ...
            ToProductDetail({ orderId, cashPay, token, channel, sourcePlatform, producType: 'coupon', couponPrice })
        } else {
            layer.open({
                content: res.message,
                skin: 'msg',
...
$(function () {
    ...
    // 购买按钮
    $('#shopBtn, #shopBtn1,#shopBtn2, #shopBtn3').on('click', function () {
    ...
    $('#infoTitle,.oldForNew-product-info-4 ,.view-detail').on('click', function () {
        window.open(`../productDetail/guessProductDetail2.html?productId=${productInfo.id}`, '_self')
    })
    $(document).on('click', '.coupon-bg', function () {
        layer.open({
            content: '领取成功',
            skin: 'msg',
            time: 1
        });
        $('.coupon-bg').addClass('active')
        $('#getBtn').text('已领取')
    })
    $(document).on('click', '#closeCoupon', function () {
    ...
    // 导航
    $('#backTop2').on('click', function () {
        $('.back-fixed').addClass('ani')
    })
    $('.back-close').on('click', function () {
        $('.back-fixed').removeClass('ani')
    })
    // 去领取会员卡
