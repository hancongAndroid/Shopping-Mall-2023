...
if (productType == 2) {
	$('.buy-txt.type1').addClass('dn')
	$('.buy-txt.type2').removeClass('dn')
}
...
function getProductInfo() {
	http('GET', `/mallv2/product/getProductInfoById`, {
		...
		// 信息栏目
		$('#priceYuan, #priceYuanPopup').text(productInfo.curPrice)
		$('#priceBuy').text(productInfo.price)
		$('#priceReal, .priceReal').text(productInfo.price)
		...
		$('#volumeStr, .volumeStr').text(volumeStr(productInfo.volumeStr))
...
function getRecommendOther() {
	http('GET', `/mallv2/h5/shoppingcard/queryListByPage`, {
		...
		$.each(recommond, (i, n) => {
			if (i < 3) {
				recommond1 += `<div class="product-item" onclick="window.open('./cardProductDetail.html?productId=${n.id}', '_self')">
								...
								<div class="g-product-info">
									...
									<div class="integral-price">
										<div class="price">
											<span class="price1">￥<span class="num">${n.curPrice}</span></span>
				...
				recommond2 += `<div class="product-item" onclick="window.open('./cardProductDetail.html?productId=${n.id}', '_self')">
								<img class="product-image" src="${n.productImg}" />
								<div class="g-product-info">
									<div class="descr">${n.productName}</div>
									<div class="integral-price">
...
$(function () {
	...
	// 加载客服弹窗
	loadContactServicePopup('#loadContactServicePopup', {
		'enterPage': productId,
		'enterType' : 1,
	})
