...
(function() {
	...
	document.head.appendChild(function() {
		...
		link.rel = "styleSheet"
		link.href = location + "useItem.css"
		return link
	}())
})()

// 商品数据渲染
function productListUse(productList, element) {
	let html = ''
	$.each(productList, (index, item) => {
		...
		if (item.stock == 0) {
			...
			oDiv = `<div class="minbox" onclick="window.open('/uni/#/pages/productDetail/productDetail?id=${item.id}', '_self')">`
		}
		html += `${oDiv}
