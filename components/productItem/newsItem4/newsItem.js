...
// link.rel = 'stylesheet'
// link.href = '../../components/productItem/newsItem/newsItem.css'
...
// 							<div class="midtor">￥${item.originalPrice}</div>
// 							<div class="midtjs">仅剩<span class="num">${filterStock(item.virtualStock)}</span>件</div>
// 							<div class="midtjz">
// 								<div class="midtjz1">补贴额度</div>
// 								<div class="midtjz2"><span class="num">${item.activityMax}</span>元</div>
// 							</div>
// 						</div>
// 						<div class="price">
// 							<div class="price1">
// 								<div class="price12">￥<span class="num">${item.curPrice}</span></div>
// 							</div>
	...
	$.each(productList, (index, item) => {
		...
		html += `${oDiv}
					...
					<div class="product-info">
						<div class="descr">${item.productName}</div>
						<div class="midt">
							<img class="midtImg" src="https://img.quanminyanxuan.com/other/48edbc09f6d7439a8c96f0190d609d5f.png" >
							<div class="midtor">￥${item.curPrice}</div>
							<div class="midtjs">恢复￥${item.originalPrice}</div>
							<div class="midtjz">
								<div class="midtjz1">补贴额度</div>
								<div class="midtjz2"><span class="num">${item.activityMax}</span>元</div>
							</div>
						</div>
						<div class="price">使用补贴换购></div>
					</div>
				</div>`
	})
	$(element).append(html)
